...
## Open follow-ups (from the 2026-06-16 combined UI+Bug scan, Wave 19 — Low-sev tail)
Wave 19 closed the 5 diminishing-tail findings: blueprint usage_count decoupled from the detail GET (new fire-once POST `…/view` route + ref-guarded `trackBlueprintView`); find-youtube given a `responseJsonSchema` + code-fence strip (was a false "no video found" on fenced Gemini output); achievement *looping* ambient motion gated on `allowAmbient` (glow/trophy/rings/toast/card sparkles — confetti was already gated in W15); faceted `HierarchicalNodeItem` made path-aware (threaded expandedNodes/onToggleNode/onSelectValue/onDrillDown through recursion, full `parent/child` path keys — fixes both unreachable depth and the bare-leaf-value selection that never matched the index); streak day math made timezone-aware + DST-safe (`Intl.DateTimeFormat` in the client's zone, calendar-subtraction for "yesterday" instead of now-24h, timeZone threaded through submit + streaks POST).
- Anti-patterns: a counter mutated inside a detail GET over-counts on refetch/remount (keep GET idempotent; fire-once write, ref-guarded vs StrictMode); a "daily" concept from `toISOString()` is UTC not the user's day, and "yesterday" is calendar math (subtract a calendar day from a date-only value), never `now - 24h` (breaks across DST 23h/25h days); `responseMimeType:'application/json'` alone doesn't guarantee parseable JSON once a Gemini tool is attached — pass `responseJsonSchema` AND strip fences; gate *looping* ambient motion, not just one-shot celebrations, on the reduced-motion tier.
- Structural fact: `FacetExtractor` builds only 2 hierarchy levels today and keys child `selected` on `${parentValue}/${childValue}`; the FacetPanel now selects/expands by that same full path. The challenges submit endpoint has no client caller (built-but-unwired) — the streak fix is correct-by-construction once a client sends its zone.
- Cumulative Waves 1–19: 81 functional findings addressed + 4 security mitigated + 4 decisions closed, TS held at 53, 0 regressions. Remaining: infra/security (RLS, api_keys, guest tokens, unapplied counter-RPC migration) only.
