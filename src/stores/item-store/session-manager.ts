...
export function updateSessionTimestamp(session: ListSession): ListSession {
  return {
    ...session,
    updatedAt: new Date().toISOString(),
  };
...
export function markSessionSynced(session: ListSession): ListSession {
  return {
    ...session,
    synced: true,
    updatedAt: new Date().toISOString(),
...
export function validateSession(session: ListSession): boolean {
  return !!(
    session.id &&
    session.listId &&
    session.gridItems &&
...
export function hasUnsavedChanges(session: ListSession): boolean {
  const lastUpdate = new Date(session.updatedAt);
  const fiveMinutesAgo = new Date(Date.now() - 5 * 60 * 1000);
  return lastUpdate > fiveMinutesAgo && !session.synced;
}

export function getSessionMetadata(session: ListSession) {
  const progress = calculateProgress(session.gridItems);
  return {
    id: session.id,
    ...
    hasUnsavedChanges: hasUnsavedChanges(session),
  };
}
