    ...
    const initializeData = async () => {
      try {
        ...
        if (currentList?.id === listId) {
          ...
          // Always initialize backlog — initializeGroups has its own cache
          // validation that checks whether items are actually loaded, not just
          // whether group metadata exists in the store.
          if (currentList.category) {
            await initializeGroups(currentList.category, currentList.subcategory);
          }
...
/**
 * Goat Page - Main ranking interface
 *
 * Handles:
 ...
 *
 * Access at: /goat?list={listId}
 */
export default function GoatPage() {
  return (
    <div data-testid="goat-page">
      <Suspense fallback={
        <div className={PAGE_CONTAINER_CLASS}>
          <div className="text-center">
            <div className={SPINNER_CLASS}></div>
